...
describe('flag and survey storage split', () => {
    ...
    describe('a stale group entry does not overwrite a fresher main blob', () => {
        ...
        // No timestamp on either side (e.g. data written by a pre-freshness-stamp
        // SDK): the group entry remains the canonical migrated-forward home.
        it('surveys: the group entry wins when neither side carries a freshness stamp', () => {
            localStorage.setItem(MAIN, JSON.stringify({ [SURVEYS]: [{ id: 'main' }], distinct_id: 'd' }))
            localStorage.setItem(SURVEYS_ENTRY, JSON.stringify({ [SURVEYS]: [{ id: 'group' }] }))
